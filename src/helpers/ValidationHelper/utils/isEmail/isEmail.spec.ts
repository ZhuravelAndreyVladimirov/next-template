import { isEmailFunc } from "./isEmail";

describe("ValidationHelper isEmailFunc валидные email", () => {
  function isTrue(mail: string, name: string = "") {
    it(name, () => expect(isEmailFunc(mail)).toBe(true));
  }
...
describe("Validation helper isEmailFunc невалидные email", () => {
  function isFalse(mail: string, name: string = "") {
    it(name, () => expect(isEmailFunc(mail)).toBe(false));
  }
